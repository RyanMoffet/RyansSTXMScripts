...
if isempty(peakregion1) 
    ...
    S.LabelMat=[];
    beep
    disp('Stack does not contain the required energies for full spectrum mapping');
    return
end
...
%     [284.75,285.9],[284.75,285.9],[energy(1),283],1);
Tmp=mean(stack(:,:,ECpeakpos),3);
Tmp((Tmp-Preedge)<0)=0;
Tmp(Tmp<(DetectThresh-Preedge))=0;
% Tmp=Tmp-Preedge;
Tmp(Tmp>0)=1;
Tmp(Preedge>1.7)=0;
...
sp2out=ACC./Atot.*8.6; %% value from hopkins et al.
...
plotIn=DiffMap(:,:,1);
plotIn(plotIn>3)=3;
imagesc(xdat,ydat,plotIn),hold on
for k = 1:length(B)
    boundary = B{k};
...
subplot(3,2,2)
imagesc(xdat,ydat,DiffMap(:,:,2)),hold on
for k = 1:length(B)
    boundary = B{k};
    plot(boundary(:,2).*(S.Xvalue./xmax), ...
        boundary(:,1).*(S.Yvalue./ymax), 'w', 'LineWidth', 2)
...
imagesc(xdat,ydat,sp2out),hold on
for k = 1:length(B)
    boundary = B{k};
    plot(boundary(:,2).*(S.Xvalue./xmax), ...
        boundary(:,1).*(S.Yvalue./ymax), 'w', 'LineWidth', 2)
...
subplot(3,2,4)
imagesc(xdat,ydat,DiffMap(:,:,5)),hold on
for k = 1:length(B)
    boundary = B{k};
    plot(boundary(:,2).*(S.Xvalue./xmax), ...
        boundary(:,1).*(S.Yvalue./ymax), 'w', 'LineWidth', 2)
...
subplot(3,2,5)
imagesc(xdat,ydat,DiffMap(:,:,4)),hold on
for k = 1:length(B)
    boundary = B{k};
    plot(boundary(:,2).*(S.Xvalue./xmax), ...
        boundary(:,1).*(S.Yvalue./ymax), 'w', 'LineWidth', 2)
...
LabelMat=raw2mask(S);hold on
% for k = 1:length(B)
%     boundary = B{k};
%     plot(boundary(:,2).*(S.Xvalue./xmax), ...
%         boundary(:,1).*(S.Yvalue./ymax), 'w', 'LineWidth', 2),hold on,
% end
% LabelMat=raw2mask(S);hold on
