function CropPartRGBPlot(DirLabelS,sample)
...
for i=1:n
    ...
    image(xdat,ydat,uint8(RGBIn{i}),'ButtonDownFcn',{@DispDataFile,[PartDirs(i,:),PartSN(i)...
        FileLoc,FileName]});% plot image and pass file info into callback fcn
...
elseif ActionItem==2
    cd(FileInfo{4})
    load(FileInfo{5});
    if ~isfield(DirLabelS,'ToDelete')
        DirLabelS.ToDelete=FileInfo{3};
        save(sprintf('%s.mat',FileInfo{5}),'DirLabelS');
    else
        DirLabelS.ToDelete=[DirLabelS.ToDelete,FileInfo{3}];
        save(sprintf('%s.mat',FileInfo{5}),'DirLabelS')
    end
end