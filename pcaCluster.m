function [pcaMap,C,eImages]=pcaCluster(S,lowestorder,highestorder,g,nofC)
...
for k=1:nofC
    ...
    buffer=V(:,:,cnt);
    temp_spec(cnt,1)=mean(mean(buffer(pcaMap==k)));
    clear buffer
...
xlim([280,320]);
ylim([0,j+.1]);
vline([285.1,287.7,288.3,288.7,289.5,290.4,291.5],'k:')
hold off
