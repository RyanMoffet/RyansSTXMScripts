...
Thresh=[0.15,0.15,0.08,0.1,0.05];  %% Thresholds for Inorg, K, EC, TotC, carbox
particle=S.particle;
...
for i=[5,1,2,3] %% loop over chemical components
    BinCompMap{cnt}=ConstThresh(S.DiffMap(:,:,i),Thresh(i)); %% Give binary matrix with labels greater than specified threshold
        ...
        if ~isempty([a1,b1]) && ~isempty([a2,b2])
            ...
            if length(IdxCom)>0.02*length(linidx1) %% if component makes up greater than 2% of the pixels of the particle...
                if j==4 %% if it contains sp2 peak...
                    ...
                    if sp2out > 0.7 %% if %sp2 is greater than 50%, label as EC
