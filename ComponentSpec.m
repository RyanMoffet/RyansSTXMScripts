function [specout,npix]=ComponentSpec(Sin,comp,figflag)
...
if isempty(exclude) %% if we arent excluding any regions...
    if length(cmp)==1 %% if we have selected one region to analyze
        cmpidx=linidx{cmp}; %% get the indices of the component and return them into cmpidx
    else %% not excluding regions
        for i=1:length(cmp) %% loop through components
            cmpidx=union(cmpidx,linidx{i}); %% get indices of regions
        end
    end
else
    for i=exclude  %% if we are excluding regions...
        if i==cmp || isempty(linidx{i}) || i==6
            continue
        else
            tidx=intersect(linidx{cmp},linidx{i}); %% this gives the overlapping pixels of the component and the 
            %             it=crap;
            %             it(tidx)=1;
            %             figure, imagesc(it);
...
for j=1:length(Energy)
    junkmat=Spectr(:,:,j);
%     Partspec(j)=mean(mean(junkmat(cmpidx))); %% this will give mean OD per pixel
    Partspec(j)=sum(sum(junkmat(cmpidx)));  %% this will give the total OD per particle
end
...
%% Return output
npix=length(cmpidx); %% gives number of pixels in particle region
