...
function [Rad,cx,cy]=max_inscribed_circle_STXM(Sin, display)
[B,L]=bwboundaries(Sin.binmap);
sz=size(Sin.binmap);
Rad=zeros(length(B),1);
for i=1:length(B)
    
    % get the contour
    % figure,imagesc(Sin.binmap);
    ContourImage=zeros(sz);
    linB=sub2ind(sz,B{i}(:,1),B{i}(:,2));
    ContourImage(linB)=1;
    % figure,imagesc(ContourImage);
...
%     tic();
    ...
    [R RInd]=max(BW(ind));
    Rad(i)=R(1); RInd=RInd(1); % handle multiple solutions: Just take first.
...
%     toc();
    ...
    if (display)
        ...
        hold on
        plot(cos(theta).*Rad(i)+cx,sin(theta).*Rad(i)+cy,'color','g','LineWidth', 2);
    end
end
end
