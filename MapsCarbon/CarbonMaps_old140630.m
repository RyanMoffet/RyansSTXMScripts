...
test=energy(energy<320 & energy>278);
if isempty(test)
    beep
...
%         if %%soot
%             %% select inclusions
%             %% convex poly of each inclusion
%             %% aera of each inclusion
%         end
    end
...
%% Soot Region Props
SootBW=finSp2mask;
% figure,imagesc(SootBW),colormap gray
ImStruct=regionprops(SootBW,'Eccentricity','MajorAxisLength','MinorAxisLength','ConvexArea');
...
CvxArea=reshape([ImStruct.ConvexArea],size(ImStruct)).*(XSiz*YSiz);
sootIncArea=CompSize(CompSize(:,3)>0,3);
SootConvexity=sootIncArea./CvxArea;
Snew.SootEccentricity=Ecc;
Snew.SootMajorAxisLength=Maj;
Snew.SootMinorAxisLength=Min;
Snew.SootConvexity=SootConvexity;
