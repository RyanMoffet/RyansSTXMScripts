function [LabelCnt,size,label,CmpSiz,SootCarbox,TotalCarbon,Carbox,Sp2,OutRad,RadStd,SingRad,SootDistCent,SootEcc,SootMaj,SootMin,SootConvexity]=...
...
for j=1:length(InDir)  %% loop over different sample directories
    ...
    for l=1:length(directory)  %% loop over stacks
        ...
        if ~isempty(ind)
            ...
            SootConvexity{j}=[SootConvexity{j};Sinp.SootConvexity];
