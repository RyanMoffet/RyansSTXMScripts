function CropPartRGBPlot(RGBIn,ImProps,PartDirs,PartSN)
...
for i=1:n
    ...
    x(i)=imsiz(1).*XSiz(i);% x dimension of image = # pixels*pixel size
    y(i)=imsiz(2).*YSiz(i);% y dimension of image = # pixels*pixel size
end
x=x';y=y';
...
for i=1:n
    ...
    image(xdat,ydat,uint8(RGBIn{i}),'ButtonDownFcn',{@DispDataFile,[PartDirs(i,:),PartSN(i)]});% plot image and pass file info into callback fcn
