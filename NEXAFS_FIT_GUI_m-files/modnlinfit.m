function [beta,r,J,Sigma,mse] = modnlinfit(X,y,model,beta,options,par,struct)
%   modnlinfit uses the same code as nlinfit. The only difference is that
%   it can process the two additional input arguments "par" and "struct".
%   Hereby it makes the "model" used more flexible and enables it to work
%   with changing user input from the NEXAFS_FIT_GUI.
%
...
model = fcnchk(model);
try
    yfit = model(beta,X,par,struct); % modified for nexafs_fit
catch ME
    if isa(model, 'inline')
...
if strcmp(options.Robust,'off')
    ...
    [beta,J,lsiter,cause] = LMfit(X,y, model,beta,options,par,struct,verbose,maxiter); % modified for nexafs_fit
else
    ...
    % limit.
    [beta_ls,J] = LMfit(X,y, model,beta,options,par,struct,0,maxiter); % modified for nexafs_fit
    res = y - model(beta_ls,X,par,struct); % modified for nexafs_fit
    res(isnan(res)) = [];
...
%----------------------------------------------------------------------
function  [beta,J,iter,cause] = LMfit(X,y, model,beta,options,par,struct,verbose,maxiter) % modified for nexafs_fit
...
while iter < maxiter
    ...
    J = getjacobian(beta,fdiffstep,model,X,yfit,nans,par,struct); % modified for nexafs_fit
...
% ---------------------- Jacobian
function J = getjacobian(beta,fdiffstep,model,X,yfit,nans,par,struct) % modified for nexafs_fit
p = numel(beta);
...
for k = 1:p
    ...
    yplus = model(beta+delta,X,par,struct); % modified for nexafs_fit
    dy = yplus(:) - yfit(:);
    dy(nans) = [];
    J(:,k) = dy/delta(k);
    delta(k) = 0;
