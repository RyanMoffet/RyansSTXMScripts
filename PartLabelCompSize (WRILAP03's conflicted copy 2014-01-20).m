...
%% varargin is a flag for plotting: if varargin = 1, the function will plot and save
%%                                              = 2, the function will plot
%%                                              if varargin is empty, figsav will be set to 2 (it will plot only)
%% OUTPUT NOTES:
...
if isempty(S.DiffMap)
    beep
    disp('I didnt find no stinkin DiffMap!!');
    return
end
% figsav=0;
if isempty(varargin) %% set default figsav==2 (function will plot figs without saving)
    figsav=2;
end
particle=S.particle;
...
cnt=1;
%% Threshold Diffmaps to make binary composition maps
...
%% Generage average particle spectra and make figure
S.BinCompMap=temp;
S=MultPartAvSpec(S,figsav);

%% add figure to current window
if figsav==2
